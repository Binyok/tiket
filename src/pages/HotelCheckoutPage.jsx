...
const HotelCheckoutPage = () => {
  ...
  const { hotel, room, nights = 1, guests = 2, checkIn, checkOut } = state;
      ...
      <div className="bg-gray-50 min-h-screen flex items-center justify-center">
        <div className="bg-white rounded-xl shadow-md p-8 max-w-md text-center">
          <p className="text-gray-700 mb-4">
            Data pemesanan hotel tidak ditemukan. Silakan pilih hotel dan kamar terlebih dahulu.
          </p>
          <button
            className="btn-primary"
            onClick={() => navigate('/hotels')}
          >
            Kembali ke Pencarian Hotel
          </button>
        </div>
      </div>
    );
  ...
  const subtotal = basePricePerNight * nightsCount;
  ...
  const bookingDetails = {
    ...
    nights: nightsCount,
    guests,
    checkIn: checkIn || '18 Nov 2025',
    checkOut: checkOut || '20 Nov 2025',
  ...
  const guestIndexes = Array.from({ length: bookingDetails.guests }, (_, idx) => idx + 1);
    ...
    <div className="bg-gray-50 min-h-screen py-8">
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Checkout Hotel</h1>
            <p className="text-gray-600">Lengkapi pembayaran untuk menyelesaikan pemesanan hotel Anda</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                {guestIndexes.map((guest, index) => (
                  <div key={guest} className="mb-6 last:mb-0">
                    <h3 className="font-semibold text-gray-900 mb-3">Tamu {guest}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                          Nama Lengkap *
                    ...
                    {index < guestIndexes.length - 1 && <hr className="mt-6" />}
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white rounded-xl shadow-md p-6 sticky top-24">
                ...
                <div className="space-y-3 mb-6 pb-6 border-b">
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <span className="text-sm font-medium text-gray-900 text-right">{bookingDetails.hotelName}</span>
                  </div>
                  <div className="flex items-start justify-between">
                    <span className="text-sm text-gray-600">Lokasi</span>
                    <span className="text-sm font-medium text-gray-900 text-right">{bookingDetails.location}</span>
                  </div>
                  <div className="flex items-start justify-between">
                    <span className="text-sm text-gray-600">Check-in</span>
                    <span className="text-sm font-medium text-gray-900">{bookingDetails.checkIn}</span>
                  </div>
                  <div className="flex items-start justify-between">
                    <span className="text-sm text-gray-600">Check-out</span>
                    <span className="text-sm font-medium text-gray-900">{bookingDetails.checkOut}</span>
                  </div>
                  <div className="flex items-start justify-between">
                    <span className="text-sm text-gray-600">Durasi</span>
                    <span className="text-sm font-medium text-gray-900">{bookingDetails.nights} malam</span>
                  </div>
                  <div className="flex items-start justify-between">
                    <span className="text-sm text-gray-600">Tamu</span>
                    <span className="text-sm font-medium text-gray-900">{bookingDetails.guests} orang</span>
                  </div>
                  <div className="flex items-start justify-between">
                    <span className="text-sm text-gray-600">Tipe Kamar</span>
                    <span className="text-sm font-medium text-gray-900 text-right">{bookingDetails.roomName}</span>
                  </div>
                </div>
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">
                      Harga per malam x {bookingDetails.nights}
                    </span>
                    <span className="text-sm font-medium text-gray-900">
                      {formatPrice(bookingDetails.pricePerNight * bookingDetails.nights)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
